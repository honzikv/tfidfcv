...
def get_czech_documents_example():
    ...
    with open('resources/czech_documents.json', 'r', encoding='utf-8') as f:
        return list(map(lambda x: Document(doc_id_service.get_id(), x['text'].split(' ')), json.load(f)))
...
cosine_similarity = CosineSimilaritySearch(tf_idf, czech_documents, SplitPreprocessor())
